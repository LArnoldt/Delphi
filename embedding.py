...
out_dir = 'Delphi-SingleCell'
device = 'cuda' # examples: 'cpu', 'cuda', 'cuda:0', 'mps', etc.
...
def get_embeddings():
    ...
    for split in ['train', 'val']:
        ...
        for idx in tqdm(range(len(p2i))):
            idx = np.array([idx]) #.to(device)
            X, A, Y, B = get_batch(idx, data, p2i, block_size=block_size, device=device,
                                padding='random', lifestyle_augmentations=True, select='left',
                                no_event_token_rate=no_event_token_rate)
            with ctx:
                _, _, _, x = model(X, A, Y, B, validation_loss_mode=True)
            #mask = (idx != 0).unsqueeze(-1)  # (batch_size, seq_len, 1)
            #patient_emb = (x * mask).sum(1) / mask.sum(1)
            mask = (X != 0).unsqueeze(-1)
            patient_emb = (x * mask).sum(1) / mask.sum(1)  # (1, C)
            emb_list.append(patient_emb.cpu().detach().numpy())
            #emb_list.append(x.cpu().detach().numpy())
            eid_list.append(data[p2i[idx][0][0]][0])
            split_list.append(split)
    emb = np.concatenate(emb_list, axis=0)
    eid = np.array(eid_list)
    split = np.array(split_list)
    return emb, eid, split

emb, eid, split = get_embeddings()
np.save("delphi_emb.npy", emb)
np.save("delphi_eid.npy", eid)
np.save("delphi_split.npy", split)
